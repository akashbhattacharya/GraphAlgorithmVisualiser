...
\toprule
Queue Elements<xsl:text disable-output-escaping="yes"><![CDATA[&]]></xsl:text><xsl:for-each select="dijkstra/graph/vertices/vertex"><xsl:sort select="id" order="ascending"></xsl:sort><xsl:value-of select="id"></xsl:value-of><xsl:text disable-output-escaping="yes"><![CDATA[&]]></xsl:text></xsl:for-each> \\
\midrule
...
\begin{tabular}{<xsl:for-each select="dijkstra/graph/vertices/vertex">c|</xsl:for-each>c}
\toprule
Queue Elements<xsl:text disable-output-escaping="yes"><![CDATA[&]]></xsl:text><xsl:for-each select="dijkstra/iterations/iteration"><xsl:sort select="vertex/id" order="ascending"></xsl:sort><xsl:value-of select="visited_vertex/id"></xsl:value-of><xsl:text disable-output-escaping="yes"><![CDATA[&]]></xsl:text></xsl:for-each> \\
\midrule
-<xsl:text disable-output-escaping="yes"><![CDATA[&]]></xsl:text>0.0<xsl:text disable-output-escaping="yes"><![CDATA[&]]></xsl:text><xsl:for-each select="dijkstra/iterations/iteration"><xsl:if test="position()!=last()" >\infty<xsl:text disable-output-escaping="yes"><![CDATA[&]]></xsl:text></xsl:if></xsl:for-each>
\midrule
<xsl:for-each select="dijkstra/iterations/iteration"><xsl:value-of select="iteration_number"></xsl:value-of><xsl:text disable-output-escaping="yes"><![CDATA[&]]></xsl:text><xsl:for-each select="distances/distance"><xsl:sort select="vertex" order="ascending"></xsl:sort><xsl:choose><xsl:when test="value=123456.0">\infty</xsl:when><xsl:otherwise><xsl:value-of select="value"></xsl:value-of></xsl:otherwise></xsl:choose><xsl:text disable-output-escaping="yes"><![CDATA[&]]></xsl:text></xsl:for-each> \\
\midrule
</xsl:for-each>
\bottomrule
\end{tabular}
